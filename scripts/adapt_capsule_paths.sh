...
# Python: ARIAL_PATH or ARIAL_FONT_PATH
for pyscript in $(grep -rl "ix1.*Arial" "$CODE" --include="*.py" 2>/dev/null); do
  if [ "$DRY_RUN" = "1" ]; then
    echo "  WOULD FIX font: $(basename "$pyscript")"
  else
    # Replace the hardcoded path with fallback logic
    sed -i "s|ARIAL_PATH = '$ARIAL_HPC'|ARIAL_PATH = '$LIBERATION' if os.path.exists('$LIBERATION') else None|g" "$pyscript"
    sed -i "s|ARIAL_FONT_PATH = '$ARIAL_HPC'|ARIAL_FONT_PATH = '$LIBERATION' if os.path.exists('$LIBERATION') else None|g" "$pyscript"
    sed -i "s|arial_path = '$ARIAL_HPC'|arial_path = '$LIBERATION' if os.path.exists('$LIBERATION') else None|g" "$pyscript"
    CHANGED=$((CHANGED + 1))
...
# 03_wes: VCF input path
do_sed "$CODE/03_rat_wes/02_cosmic_signatures.py" \
  '/ix1/alee/LO_LAB/Personal/Alexander_Chang/alc376/NeilRatWES/RatWES_Mutect2_VCF_Input' \
  'DATA_DIR / "rat_wes"'

# 04: Xu atlas path
do_sed "$CODE/04_human_scrnaseq/00b_preprocess_seurat.R" \
  '/ix1/alee/LO_LAB/General/Public_Data/BC-Datasets/Xu_etal_Primary_Breast_Tumor_Atlas_2024' \
  'file.path(DATA_DIR, "human_scrnaseq")'

# 05: Rahul validation dir
do_sed "$CODE/05_rat_bulk_rnaseq/05_validate_vs_rahul.R" \
  '/ix1/alee/LO_LAB/Personal/Rahul/Neil_RNAseq' \
  'file.path(DATA_DIR, "rat_bulk_rnaseq", "rahul_validation")'

# 07: Neil project path + FASTQ paths
do_sed "$CODE/07_organoid_single_cell/_config.py" \
  '/ix1/alee/LO_LAB/Personal/Alexander_Chang/alc376/NeilOrganoidSingleCell' \
  'DATA_DIR / "organoid_single_cell"'
